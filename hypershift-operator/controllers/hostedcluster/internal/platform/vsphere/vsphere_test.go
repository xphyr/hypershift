...
func TestReconcileCAPIInfraCR(t *testing.T) {
	...
	expectedResult := &capivsphere.VSphereCluster{
		...
		Status: capivsphere.VSphereClusterStatus{
			Ready: true,
		},
	}
	testCases := []struct {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := vsphere.ReconcileCAPIInfraCR(
				context.Background(),
				fakeClient,
				createOrUpdateFN,
				hcluster,
...
func TestReconcileCredentials(t *testing.T) {
	...
	hcluster := &hyperv1.HostedCluster{}
	err := vsphere.ReconcileCredentials(context.Background(), fakeClient, createOrUpdateFN, hcluster, "controlPlanNamespace")
	if err != nil {
...
func TestReconcileSecretEncryption(t *testing.T) {
	...
	hcluster := &hyperv1.HostedCluster{}
	err := vsphere.ReconcileSecretEncryption(context.Background(), fakeClient, createOrUpdateFN, hcluster, "controlPlanNamespace")
	if err != nil {
