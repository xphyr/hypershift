...
type PlatformSpec struct {
	...
	// VSphere defines VSphere specific settings for cluster components.
	//
	// +optional
	// +immutable
...
type VSpherePlatformCredentials struct {
	// InfraKubeConfigSecret is a reference to a secret that contains the kubeconfig for the external infra cluster
	// that will be used to host the KubeVirt virtual machines for this cluster.
	//
	// +immutable
...
// VSpherePlatformSpec specifies configuration for kubevirt guest cluster installations
// +kubebuilder:validation:XValidation:rule="!has(oldSelf.generateID) || has(self.generateID)", message="VSphere GenerateID is required once set"
type VSpherePlatformSpec struct {
	...
	// the same cluster and namespace as the Hosted Control Plane.
	// +optional
	Credentials *VSpherePlatformCredentials `json:"credentials,omitempty"`
}
